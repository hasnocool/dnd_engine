...
This section is authoritative for how engine reads and validates `rules_data/*`.
Use these exact paths, sections, and item identities when implementing loaders, lints, and runtime resolution.

### 1) `rules_data/registry.json`

Required top-level sections:

- `version` (current value: `"0.1.0"`)
- `default_source_id` (current value: `"srd-5.2.1"`)
- `sources` (array)
- `catalogs` (object)
- `catalogs.spells` (array)

Current required items:

- `sources[0].id = "srd-5.2.1"`
- `sources[0].path = "sources/srd-5.2.1.source.json"`
- `catalogs.spells[0].source_id = "srd-5.2.1"`
- `catalogs.spells[0].path = "catalogs/spells.srd-5.2.1.json"`

### 2) `rules_data/sources/srd-5.2.1.source.json`

Required sections/keys:

- `source_id` (must equal registry `sources[*].id`)
- `source_type`
- `version`
- `license`
- `publisher`
...
### 3) `rules_data/catalogs/spells.srd-5.2.1.json`

Required top-level sections:

- `catalog_type` (must be `"spells"`)
- `source_id` (must equal registry catalog `source_id`)
- `version`
- `spells` (array)

Current spell items in `spells[]`:

- `spells[0].id = "spell.fire_bolt"`
- `spells[1].id = "spell.acid_splash"`

Canonical spell fields consumed by engine:

- Identity/provenance: `id`, `name`, `source_id`, `level`, `school`, `resolution_mode`
- Attack resolution fields: `attack_bonus`, `damage`, `damage_type`, `is_melee`, `range_feet`
- Save resolution fields: `save_ability`, `save_dc`, `half_damage_on_save`
- Condition fields: `condition_on_failed_save`, `condition_duration_rounds`
- Metadata: `notes`

### 4) Encounter -> `rules_data` linkage contract

For every action where `kind == "spell"`:

- `spell_ref` must match one `spells[*].id` from loaded spell catalogs
- If canonical spell has no `attack_bonus` and `resolution_mode == "attack"`, encounter action must set `attack_bonus_override`
- If canonical spell has no `save_dc` and `resolution_mode == "save"`, encounter action must set `save_dc_override`

Optional encounter override fields mapped to canonical spell data:
...
### 5) Code modules that must reference these sections/items

- `src/rules_registry.py`: loads `registry.json`, `sources[*]`, `catalogs.spells[*]`
- `src/spell_catalog.py`: loads `catalogs/*` and indexes `spells[*].id`
- `src/rules_linter.py`: validates section/key presence, id cross-links, duplicate rule ids, and encounter spell references
- `src/turn_engine.py`: resolves `spell_ref` to canonical `spells[*]` fields and applies encounter overrides
...
- `registry.sources[*].id`
- `registry.catalogs.spells[*].source_id`
- `source file source_id`
- `catalog file source_id`
- each `spell.source_id`

Any mismatch should be treated as `bad_source_reference` by linting and blocked from release.
